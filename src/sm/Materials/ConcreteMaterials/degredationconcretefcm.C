...
namespace {
double evaluateDegreeOfDegradation(oofem::Domain *domain, oofem::GaussPoint *gp, oofem::ValueModeType mode,  oofem::TimeStep *tStep)
{
...
namespace oofem {
REGISTER_Material(DegredationConcreteFCM);

DegredationConcreteFCM :: DegredationConcreteFCM(int n, Domain *d) : FCMMaterial(n, d), RandomMaterialExtensionInterface()
{}
...
DegredationConcreteFCM :: initializeFrom(const std::shared_ptr< InputRecord > &ir)
{
    ...
    if ( beta_Gf < 0. || beta_Gf > 1. ) {
        throw ValueInputException(ir, _IFT_ConcreteFCM_beta_Gf, "Parameter beta_Gf must be between 0 and 1");
    }
    if ( beta_f < 0. || beta_f > 1. ) {
        throw ValueInputException(ir, _IFT_ConcreteFCM_beta_f, "Parameter beta_f must be between 0 and 1");
    }
    if ( beta_E < 0. || beta_E > 1. ) {
        throw ValueInputException(ir, _IFT_ConcreteFCM_beta_E, "Parameter beta_E must be between 0 and 1");
    }
...
void
DegredationConcreteFCM :: giveRealStressVector(FloatArray &answer, GaussPoint *gp,
                                               const FloatArray &totalStrain,
                                               TimeStep *tStep) const
{
    DegredationConcreteFCMStatus *status = static_cast< DegredationConcreteFCMStatus * >( this->giveStatus(gp) );

    const int stepNumber = tStep->giveNumber();
    if ( status->giveLastStressRescalingStep() != stepNumber ) {
        const double DoD = evaluateDegreeOfDegradation(domain, gp, VM_Total, tStep);
        const double incrementDoD = evaluateDegreeOfDegradation(domain, gp, VM_Incremental,  tStep);
        const double previousDoD =  DoD - incrementDoD;

        const double numerator = ( 1. - ( 1. - beta_E ) * DoD );
        const double denominator = ( 1. - ( 1. - beta_E ) * previousDoD );
        const double stressScale = numerator / denominator ;

        FloatArray rescaledStress = status->giveStressVector();
        rescaledStress.times(stressScale);
        status->letStressVectorBe(rescaledStress);
        status->setLastStressRescalingStep(stepNumber);
    }
...
double
DegredationConcreteFCM :: giveFractureEnergy(GaussPoint *gp, TimeStep *tStep) const
{
    const double DoD = evaluateDegreeOfDegradation(domain, gp,VM_Total, tStep);
    return this->give(gf_ID, gp) * ( 1. - ( 1. - beta_Gf ) * DoD );
}


double
DegredationConcreteFCM :: giveTensileStrength(GaussPoint *gp, TimeStep *tStep) const
{
    const double DoD = evaluateDegreeOfDegradation(domain, gp,VM_Total, tStep);
    return this->give(ft_strength, gp) * ( 1. - ( 1. - beta_f ) * DoD );
}


double
DegredationConcreteFCM :: computeOverallElasticStiffness(GaussPoint *gp, TimeStep *tStep) const
{
    const double DoD = evaluateDegreeOfDegradation(domain, gp,VM_Total, tStep);
    return linearElasticMaterial.giveYoungsModulus() * ( 1. - ( 1. - beta_E ) * DoD );
}

// double 
// DegredationConcreteFCM :: computeOverallElasticShearModulus(GaussPoint *gp, TimeStep *tStep) const
// {
//     const double DoD = evaluateDegreeOfDegradation(domain, gp, tStep);
//     return linearElasticMaterial.giveShearModulus() * ( 1. - ( 1. - beta_E ) * DoD );
// }


double
DegredationConcreteFCM :: giveCrackingModulus(MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep, int i) const
//
...
double
DegredationConcreteFCM :: giveCrackingModulusInTension(MatResponseMode rMode, GaussPoint *gp, TimeStep *tStep, int i) const
{
  ...
  double E = this->computeOverallElasticStiffness(gp, tStep);
    ...
    if ( ( rMode == TangentStiffness ) && ( ec >= emax ) ) { // tangent and NOT unloading
        ...
        Cf = DegredationConcreteFCM :: giveNormalCrackingStress(gp, tStep, emax * Ncr, i); // gets stress
...
double
DegredationConcreteFCM :: giveNormalCrackingStress(GaussPoint *gp, TimeStep *tStep, double ec, int i) const
//
...
double
DegredationConcreteFCM :: computeEffectiveShearModulus(GaussPoint *gp, TimeStep *tStep, int shearDirection) const
...
double
DegredationConcreteFCM :: computeD2ModulusForCrack(GaussPoint *gp, TimeStep *tStep, int icrack) const
{
    ...
    if ( this->isIntact(gp, icrack) || ( this->shearType == SHR_NONE ) ) {
	  ...
	  D2 = this->sf * DegredationConcreteFCM::giveCrackingModulusInTension(SecantStiffness, gp, tStep, icrack);
            D2 /= N;
        } else if ( this->shearType == SHR_UserDefined_ShearRetFactor ) {
...
double
DegredationConcreteFCM :: computeNumerD2ModulusForCrack(GaussPoint *gp, TimeStep *tStep, int icrack) const
{
    double D2, N, E;
    ...
    if ( this->isIntact(gp, icrack) || ( this->shearType == SHR_NONE ) ) {
      E = this->computeOverallElasticStiffness(gp, tStep);
        ...
        if ( shearType == SHR_Const_ShearRetFactor ) {
	  D2 = DegredationConcreteFCM :: computeD2ModulusForCrack(gp, tStep, icrack);
        } else if ( shearType == SHR_Const_ShearFactorCoeff ) {
            // for the number of parallel cracks is taken care of in "giveCrackingModulus"
            // however, it is essential to reduce the final modulus due to the presence of multiple cracks
	  D2 = this->sf_numer * DegredationConcreteFCM::giveCrackingModulusInTension(SecantStiffness, gp, tStep, icrack);
	  D2 /= N;
        } else if ( this->shearType == SHR_UserDefined_ShearRetFactor ) {
	  D2 = DegredationConcreteFCM :: computeD2ModulusForCrack(gp, tStep, icrack);
        } else {
...
void
DegredationConcreteFCM :: checkSnapBack(GaussPoint *gp, TimeStep *tStep, int i) const
{
    DegredationConcreteFCMStatus *status = static_cast< DegredationConcreteFCMStatus * >( this->giveStatus(gp) );
...
double
DegredationConcreteFCM :: maxShearStress(GaussPoint *gp, TimeStep *tStep, int i) const {
    int dir_1, dir_2;
...
double
DegredationConcreteFCM :: computeResidualTensileStrength(GaussPoint *gp, TimeStep *tStep) const {
  ...
  for ( int i = 1; i <= nCracks; i++ ) {
    ...
    if (emax > 0.) {
      ...
      sigma = DegredationConcreteFCM :: giveNormalCrackingStress(gp, tStep, emax, i);
...
double
DegredationConcreteFCM :: give(int aProperty, GaussPoint *gp) const
{
    this->giveStatus(gp);
...
int
DegredationConcreteFCM :: giveIPValue(FloatArray &answer, GaussPoint *gp, InternalStateType type, TimeStep *tStep)
{
...
MaterialStatus *
DegredationConcreteFCM :: giveStatus(GaussPoint *gp) const
{
...
DegredationConcreteFCMStatus :: DegredationConcreteFCMStatus(GaussPoint *gp) :
...
void
DegredationConcreteFCMStatus :: printOutputAt(FILE *file, TimeStep *tStep) const
{
...
//
{
    ...
    tempElasticStiffnessScale = elasticStiffnessScale;
}


void
DegredationConcreteFCMStatus :: updateYourself(TimeStep *tStep)
//
...
//
{
    ...
    elasticStiffnessScale = tempElasticStiffnessScale;
}

Interface *
DegredationConcreteFCMStatus :: giveInterface(InterfaceType type)
{
...
void
DegredationConcreteFCMStatus :: saveContext(DataStream &stream, ContextMode mode)
{
...
void
DegredationConcreteFCMStatus :: restoreContext(DataStream &stream, ContextMode mode)
{
